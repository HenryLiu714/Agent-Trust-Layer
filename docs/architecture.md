...
| --- | --- | --- |
| 0 | `exchange` | The wire vocabulary: `Request`, `Response`, `Exchange`, the kinds, the `answered_by` values and every flag, and `is_authored_write`, the one statement of which writes enter the engine's write log - the summary files an overlaid read by the same rule (#48). |
