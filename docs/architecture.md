...
| 3 | `echo` | The body a locally answered write gets. L0: form and JSON reflection, minted ids, Slack's envelope. L1: the route's fixture with the request's own fields written over it. A Slack envelope may name a field `optional:`, which the fixture holds as `null` only so a posted value can land on it and which is dropped again when the caller posted none - real Slack sends no `message.thread_ts` for a top-level post, and Stripe really does send `reason: null`, so the rule is the envelope's and not the reflection's (#55). `fake_rejection` carries an L3 rejection's modeled error body at the level the route's write would have been answered at - there is no `fake-L3` (#45). Also `observe_read`, the one seam where a forwarded read's body teaches the faker something (a Slack `ts` a minted one must sort after). |
| 3 | `services/` | What a faked write does to a later live read, per service, as plain functions over plain data (#43). `model` is the `Read` / `Write` / `Applied` / `Rewritten` vocabulary; `stripe` and `slack` are the effects tables (#44). An effect may also answer a read outright, status and all (`Applied.status`), for the one case where the service could not have answered it: a read of an object THIS RUN minted, which it refuses because it has never heard of the id (#52). Also the L3 preconditions (#45): `Proposal` / `Probe` / `Rejection` / `NotEvaluable` / `Check` in `model`, and `PRECONDITIONS`, the table a route's `precondition:` key names an entry in - each check says which one real read it needs and what the answer, with the run's writes applied, makes of the write. A check may also name the one non-verdict fact its read carries: `Check.currency`, the code the document denominates amounts in, which is service knowledge and not a key the policy may guess at (#60). A verdict has three answers, not two: rejected, passed, and `NOT_EVALUABLE` for a document it cannot read, which is why a Slack `missing_scope` at HTTP 200 never prints as a check that passed. Pure: no clock, no minting, no I/O, so Phase 5 replay runs the same functions over a recording. Also `Idempotency` and the `IDEMPOTENCY` table (#46): the header a service keys a repeat on, and what it answers when that key comes back with different parameters. Slack has no entry, because it has no such mechanism. |
...
- **A read of an object this run minted is answered, not left as the service's refusal.** It is
